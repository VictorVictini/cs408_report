...
\item \lintexspaced must detect and recommend fixes for \gls{tagging} and lack of \gls{image_desc}.
...
\subsection{\lintexspaced supported accessibility features}\label{specification_design:supported_accessibility}
...
\item[\textbf{Table tagging}] Each table should have its tagging standard applied, with the \verb|\tagpdfsetup| command prefixing the tabular group construct, as defined in \fullref{impl:peg_parsing}, with either `table/tagging=presentation' in its required argument for tables that are read in reading order, or `table/header-rows={N}', where N is the row the headings, for tables where the heading is prefixed before stating a table cell's contents.
...
\end{enumerate}
All \verb|\includegraphics| are expected to have \gls{alt_text}, \gls{actual_text}, or \gls{artifact}.

Examples of each of these supported accessibility features can be found in \cref{appendix:accessible_doc}.

\subsection{\nmu Design \nmu patterns used}\label{specification_design:design_patterns_used}
...
\subsection{\nmu Documentation \nmu}\label{specification_design:documentation}
Thorough documentation has been provided in the form of comments in code, with extra emphasis on the \acrshort{peg} script in \cref{appendix:peg_script}. Additionally, the `README.md' contains thorough steps focused on prerequisite installations, installation steps, and steps for contributing to the project.