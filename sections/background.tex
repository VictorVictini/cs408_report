...
There were many considerations when deciding what programming language to write LinTeX in. Languages chosen were based on those that I had prior experience in, to ensure minimal hinderance from the language itself to the development of the project. An important factor is ensuring LinTeX's maintainability in the future. Therefore, only the languages which have actively maintained Parsing Expression Grammar (PEG) libraries will be considered, as can be seen in \tableref{table:language_peg_frameworks}. The most recent update times provided in \tableref{table:language_peg_frameworks} are in reference to 24\textsuperscript{th} November 2025.
...
It should be noted that while Go, C, C\#, and C++ are not directly compatible with devweb, as referenced in \tableref{table:language_web_dev}, they can be supported through use of Common Gateway Interface (CGI) scripts, and consequently should not be fully disregarded for this alone. However, based on their support for web development, C, Java, C\#, and C++ will have a lower priority for being a chosen language.
...
Go has two libraries that are used in relation to PEG systems: Pigeon by Martin Angers (mna), and peg by Andrew Snodgrass (pointlander). Direct comparisons with core components of both have been made in \tableref{table:peg_pigeon_comparison}.
